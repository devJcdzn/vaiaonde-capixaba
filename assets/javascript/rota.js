var urlAPI = "https://vaiaondecapixaba.com.br/api/rotas/"
...
function getStatus() {
  ...
  fetch(urlAPI + `?id=${id}/`)
    ...
    .then(restaurant => {
      restaurant.forEach(rest => {
        statusSpan.textContent = rest.cidade;
      })
    })
}
...
function getContact() {
  const infoPlace = document.querySelector('.info-estabelecimento');
  ...
  fetch(urlAPI + `?id=${id}/`)
    ...
    .then(restaurant => {
      restaurant.forEach(rest => {
        ...
        if (rest.instagramURL && rest.telefone && rest.temReels) {
